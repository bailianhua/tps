...
function generate(sourceLang, templateAst, isVue2, cssScopedClasses = [], htmlToTemplate, isScriptSetup) {
    const tsCodeGen = (0, code_gen_1.createCodeGen)();
    const tsFormatCodeGen = (0, code_gen_1.createCodeGen)();
    const cssCodeGen = (0, code_gen_1.createCodeGen)();
    ...
    for (const tag in tags) {
        ...
        for (const eventName in resolvedTag.events) {
            ...
            const key_2 = (0, shared_1.camelize)('on-' + key_1); // onClickOutside
            const key_3 = (0, shared_1.camelize)(key_1); // clickOutside
            ...
            writeCodeWithQuotes(key_2, event.offsets.map(offset => ({ start: offset, end: offset + eventName.length })), {
                vueTag: 'template',
                ...
                doRename(oldName, newName) {
                    ...
                    if ((0, shared_1.hyphenate)(newName).startsWith('on-')) {
                        return (0, shared_1.camelize)(hName.substr('on-'.length));
                    }
                    return newName;
                },
            });
        ...
        const name2 = (0, shared_1.camelize)(tag); // helloWorld
        ...
        tsCodeGen.addText(`// @ts-ignore\n`);
        tsCodeGen.addText(`({ `);
        writeObjectProperty2(tag, tagRanges, {
            vueTag: 'template',
        ...
        for (let i = 0; i < names.length; i++) {
            ...
            writePropertyAccess2(names[i], tagRanges, {
                vueTag: 'template',
        ...
        for (const name of componentNames) {
            tsCodeGen.addText(`${var_baseProps}.${string_1.SearchTexts.PropsCompletion(name)};\n`);
        }
        tagResolves[tag] = {
        ...
        function writeOptionReferences() {
            ...
            for (const propName in resolvedTag.props) {
                ...
                propNames.add((0, shared_1.camelize)(propName));
                for (const name of propNames.values()) {
                    // __VLS_options.props
                    tsCodeGen.addText(`// @ts-ignore\n`);
                    tsCodeGen.addText(`${var_wrapComponent}.__VLS_options.props`);
                    writePropertyAccess2(name, prop.offsets.map(offset => ({ start: offset, end: offset + prop.argName.length })), {
                        vueTag: 'template',
                        capabilities: {
                        ...
                        beforeRename: shared_1.camelize,
                        doRename: keepHyphenateName,
                    });
                    tsCodeGen.addText(`;\n`);
                }
            ...
            for (const eventName in resolvedTag.events) {
                ...
                eventNames.add((0, shared_1.camelize)(eventName));
                propNames.add((0, shared_1.camelize)('on-' + eventName));
                for (const name of eventNames.values()) {
                    // __VLS_options.emits
                    tsCodeGen.addText(`// @ts-ignore\n`);
                    tsCodeGen.addText(`${var_wrapComponent}.__VLS_options.emits`);
                    writePropertyAccess2(name, event.offsets.map(offset => ({ start: offset, end: offset + eventName.length })), {
                        vueTag: 'template',
                        capabilities: {
                            ...capabilitiesSet.attr,
                            basic: false,
    ...
    function visitElementNode(node, parentEl) {
        ...
        function writeProps(node, forRemainStyleOrClass) {
